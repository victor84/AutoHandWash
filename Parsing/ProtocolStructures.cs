...
public enum e_packet_type : byte
{
    ...
    distribute_prize,			// выдача приза терминалом
	distribute_discount_card	// выдача дисконтной карты
};
...
public struct tag_counters_packet
{
    ...
    public UInt32           osmosis;                // Средство от мошек
};
...
public struct tag_settings_packet
{
    ...
    public UInt32 frost_protection_value;	// Защита от замерзания
    public UInt32 discount_card_condition;	// Сумма для выдачи дисконтной карты
    public UInt32 pressurized_water;		// Вода по давлением
    public UInt32 water_without_pressure;	// Вода без давления
    public UInt32 foam;						// Пена	
    public UInt32 wax;						// Воск	
...
// структура пополнения баланса
public struct tag_refill_cache_packet
{
    public UInt16 cache;	// сумма для пополнения в рублях
};
...
******************************************************************************************/
// приз
public struct tag_prize_packet
{
    public UInt16 prize_size;	// размер приза
};
...
******************************************************************************************/
// состояние терминала
public struct tag_terminal_state_packet
{
    public e_terminal_state state;	// Состояние (работает, простой, сломана)
};
...
/******************************************************************************************
Структура выданного приза за один раз
Может отправляться несколько раз при прерванной выдаче приза
-------------------------------------------------------------------------------------
Название						|	Длина	|	Тип		|	Фиксированное значение	|
-------------------------------------------------------------------------------------
Статус выдачи					|	1		|	BYTE	|							|
Количество выданных денег		|	2		|	USHORT	|							|
-------------------------------------------------------------------------------------
******************************************************************************************/
// статус выдачи элемента
public enum e_distribute_element_status : byte
{
    success,		// успешная операция
    empty_device,	// пустое устройство выдачи
    error			// ошибка при выдаче
};

// выдача приза
public struct tag_distribute_prize_packet
{
    public e_distribute_element_status status;	// статус выдачи
    public UInt16 size;	// размер выданного приза
};

/******************************************************************************************
Структура выданной дисконтной карты
-------------------------------------------------------------------------------------
Название						|	Длина	|	Тип		|	Фиксированное значение	|
-------------------------------------------------------------------------------------
Статус выдачи					|	1		|	BYTE	|							|
...
// выдача приза
public struct tag_distribute_discount_card_packet
{
    public e_distribute_element_status status;	// статус выдачи
};
